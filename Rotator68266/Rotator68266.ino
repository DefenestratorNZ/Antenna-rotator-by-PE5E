...
//  Changed #include <arduino.h> to #include <Arduino.h> for bug in Arduino IDE 1.8.8 in Linux
...
#define WINDUP_LIMIT 50            //Sets the total number of degrees azimuth rotation in any direction before resetting to zero
//Motor pins - Don't change
const int azFwdPin = 14;
const int azRevPin = 12;
const int elFwdPin = 13;
const int elRevPin = 15;
//Speaker pins
const int gndPin = 7;    //Makes a convenient ground pin adjacent to the speaker pin
const int spkPin = 8;    //Attach a piezo buzzer to this pin. It beeps when new calibration data arrives.
...
//Filter constants
const float azAlpha = 0.05; //Alpha value for AZ motor filter: Decrease to slow response time and reduce motor dither.
const float elAlpha = 0.05; //Alpha value for EL motor filter: Decrease to slow response time and reduce motor dither.
const float lsmAlpha = 0.9; //Alpha value for sensor filter: Decrease to slow response time and ease calibration process.
